  ...
  return (
    ...
    <PageTransition className="bec-resources-page">
      <section className="bec-section relative overflow-hidden bg-[#fbfcfb]">
        <div className="bec-orb" style={{ bottom: '-100px', right: '10px' }} />
        <div className="bec-dot-pattern" style={{ top: '40px', left: '40px' }} />
            ...
            <aside className="w-full lg:w-64 lg:sticky lg:top-24 z-10">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-[#edf6f2] space-y-8">
                ...
                <div className="pt-6 border-t border-[#edf6f2]">
                  <h3 className="text-xs font-black uppercase tracking-widest text-[#14202d] mb-4">Search</h3>
            ...
            <div className="flex-1 w-full">
              {loading ? (
                ...
                <div className="py-20 text-center border-2 border-dashed border-gray-200 rounded-xl max-w-3xl mx-auto">
                  ...
                  <p className="text-gray-500">We are currently curating the best content for this category.</p>
                </div>
              ) : (
                  ...
                  {paginatedPosts.length > 0 && currentPage === 1 && (
                    ...
                    <Reveal y={20}>
                      <Link 
                        ...
                        className="group block bg-white rounded-[40px] overflow-hidden border border-[#edf6f2] shadow-xl hover:shadow-2xl transition-all"
                      >
                        <div className="grid grid-cols-1 md:grid-cols-2">
                          <div className="relative h-64 md:h-auto overflow-hidden">
                            <motion.img 
                              src={filteredPosts[0]?.coverImageUrl || "https://images.unsplash.com/photo-1499750310107-5fef28a66643?auto=format&fit=crop&q=80&w=800"} 
                              alt={filteredPosts[0]?.title}
                              className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                            />
                            <div className="absolute top-6 left-6">
                              <span className="bg-[#c09643] text-white px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">
                                {filteredPosts[0]?.category}
                              </span>
                            </div>
                          </div>
                          <div className="p-10 md:p-14 flex flex-col justify-center">
                            <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-[#c09643] mb-6">
                              <span className="bg-[#edf6f2] px-3 py-1 rounded-full text-[#08735d]">Featured Article</span>
                              <span>•</span>
                            ...
                            <h2 className="text-3xl md:text-5xl font-black text-[#14202d] mb-6 group-hover:text-[#08735d] transition-colors leading-tight">

                              {filteredPosts[0]?.title}
                            </h2>
                            <p className="text-[#5d6870] text-lg leading-relaxed mb-8">
                              {(filteredPosts[0]?.excerpt || filteredPosts[0]?.content?.replace(/<[^>]*>?/gm, '') || '').substring(0, 200)}...
                            </p>
                            <div className="flex items-center gap-2 font-bold text-[#08735d] group-hover:gap-4 transition-all">
                              Read Full Article <ArrowRight size={20} className="transition-transform group-hover:translate-x-2" />
                            </div>
                    ...
                    {paginatedPosts.slice(currentPage === 1 ? 1 : 0).map((post, idx) => (
                      <Reveal key={post.id} y={20} delay={idx * 0.1}>
                        <Link 
                          ...
                          className="group bg-white rounded-[32px] overflow-hidden border border-[#edf6f2] shadow-lg hover:shadow-2xl transition-all h-full flex flex-col"
                        >
                          <div className="relative h-56 overflow-hidden">
                            <motion.img 
                              src={post.coverImageUrl || "https://images.unsplash.com/photo-1499750310107-5fef28a66643?auto=format&fit=crop&q=80&w=800"} 
                              alt={post.title}
                              className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                            />
                            <div className="absolute top-4 left-4">
                              <span className="bg-[#c09643]/90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">
                                {post.category}
                              </span>
                            </div>
                          </div>
                          <div className="p-8 flex-1 flex flex-col">
                            <div className="flex items-center gap-2 text-[10px] font-black text-[#c09643] uppercase tracking-widest mb-4">
                              <Clock size={12} />
                              <span>{post.readTime} read</span>
                            </div>

                            <h3 className="text-xl font-bold text-[#14202d] mb-4 group-hover:text-[#08735d] transition-colors line-clamp-2">
                              {post.title}
                            </h3>
                            <p className="text-sm text-[#5d6870] leading-relaxed mb-6 line-clamp-3">
                              {(post.excerpt || post.content.replace(/<[^>]*>?/gm, '')).substring(0, 120)}...
                            </p>
                            <div className="mt-auto flex items-center gap-2 font-bold text-[#08735d] text-sm bec-link-underline">
      ...
      <section className="py-28 px-4">
        <div className="bec-container">
          <div className="bg-[#14202d] p-12 md:p-24 rounded-[60px] text-center text-white shadow-2xl relative overflow-hidden">
            <div className="bec-orb" style={{ top: '-100px', left: '-100px', opacity: 0.1 }} />
            <div className="relative z-10">
              <h2 className="text-[32px] md:text-[40px] font-[800] mb-6 leading-tight max-w-2xl mx-auto text-white">Never Miss a Professional Milestone</h2>
              <p className="text-white/60 text-[15px] mb-12 max-w-xl mx-auto">
                Join 10,000+ professionals getting weekly career advice, business insights, and exclusive workshop invites.
              </p>

              
              <form onSubmit={handleSubscribe} className="flex flex-col md:flex-row gap-4 max-w-lg mx-auto bg-white/5 p-2 rounded-3xl backdrop-blur-sm border border-white/10">
                <input 
                  type="email" 
                  required
                  value={email}
